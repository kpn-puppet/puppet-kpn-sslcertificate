...
Puppet::Type.type(:sslcertificate).provide(:cert) do
  ...
  def self.write_cer_to_cert_file(certificate_content)
    ...
    out_file.puts("#{certificate_content}")
    out_file.close
  end
  ...
  def self.write_base64_to_cert_file(format,certificate_content)
    ...
    content = certificate_content.gsub("\n",'').gsub(/\s+/,'')
    result = powershell("[IO.File]::WriteAllBytes('c:\\windows\\temp\\cert_file.#{format}', [Convert]::FromBase64String('#{content}'))")
  end
  ...
  def create
    ...
    result = powershell("Remove-Item c:/windows/temp/cert_file.#{resource[:format]}")
  end
  ...
  def destroy
    ...
    result = powershell("Remove-Item Cert:/#{cert_name} -DeleteKey")
  end
  ...
  def self.instances
    ...
    certificate_list = powershell("Get-ChildItem Cert:\LocalMachine -recurse | Format-list -Property pspath,subject,issuer,NotBefore,NotAfter | out-string -width 4096").gsub(/^$\n/,'')

    certificate_list.each_line do |line|
      var=line.gsub(/\n/,'').split(':')

      if var[0] == 'PSPath    ' and var[3] != nil
        name       = var[3]
        path       = var[3]
